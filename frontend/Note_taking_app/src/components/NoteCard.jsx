...
const NoteCard = ({ note, onEdit, onDelete, onToggleFavorite, onTogglePin }) => {
  return (
    <div className={`bg-white rounded-xl shadow-md overflow-hidden border ${note.isPinned ? 'border-orange-400' : 'border-gray-200'}`}>
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-bold text-lg">{note.title}</h3>
          <div className="flex space-x-2">
            <button onClick={() => onTogglePin(note.id)} className="p-1">
              <PinnedIcon className={`w-5 h-5 ${note.isPinned ? 'text-orange-500 fill-orange-500' : 'text-gray-400'}`} />
            </button>
            <button onClick={() => onToggleFavorite(note.id)} className="p-1">
              <StarIcon className={`w-5 h-5 ${note.isFavorite ? 'text-yellow-500 fill-yellow-500' : 'text-gray-400'}`} />
            </button>
          </div>
        </div>
